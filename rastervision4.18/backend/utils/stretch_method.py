...
def stretch_linear_pct2(array):
    """
    2%线性拉伸：将图像数据的最小值和最大值分别拉伸到2%和98%分位数。
    输入可以是单波段 (2D) 或 RGB (3D)。
    """
    if array.ndim == 2:
        return _stretch_linear_pct2_2d(array)
    elif array.ndim == 3:
        stretched_bands = []
        for i in range(array.shape[2]):
            stretched = _stretch_linear_pct2_2d(array[:, :, i])
            stretched_bands.append(stretched)
        return np.stack(stretched_bands, axis=2)
    else:
        raise ValueError("Unsupported array shape for stretch_linear_pct2")
    
def stretch_linear_pct5(array):
    """
    5%线性拉伸：将图像数据的最小值和最大值分别拉伸到5%和95%分位数。
    输入可以是单波段 (2D) 或 RGB (3D)。
    """
    if array.ndim == 2:
        return _stretch_linear_pct5_2d(array)
    elif array.ndim == 3:
        stretched_bands = []
        for i in range(array.shape[2]):
            stretched = _stretch_linear_pct2_2d(array[:, :, i])
...
def _stretch_linear_pct2_2d(array):
    """
    对单个波段图像进行2%线性拉伸。
    """
    ...
    min_val = np.percentile(valid_data, 2) # 获取图像2%的最小值
    max_val = np.percentile(valid_data, 98) # 获取图像98%的最大值
...
def _stretch_linear_pct5_2d(array):
    """
    对单个波段图像进行1%线性拉伸。
    """
    ...
    min_val = np.percentile(valid_data, 5) # 获取图像1%的最小值
    max_val = np.percentile(valid_data, 95) # 获取图像99%的最大值
